...
export default function RheogrammeSimulateur() {
  ...
  const lastActiveData = useRef(null);
  ...
  // ── Clic sur la courbe Recharts → dépose un point ──────────────────────────
  // On utilise activeLabel (X) et on recalcule Y depuis l'équation exacte du modèle.
  // Ainsi le point vérifie TOUJOURS l'équation mathématique, peu importe le pixel cliqué.
  // lastActiveData sert de fallback : dans certains environnements (GitHub Pages, prod),
  // activePayload peut être vide au moment du click si le hover n'a pas été mis à jour.
  const handleChartMouseMove = useCallback((data) => {
    if (data?.activePayload?.length) lastActiveData.current = data;
  }, []);

  const handleChartClick = useCallback(() => {
    if (tool !== "none") return; // laisse le canvas gérer
    const d = lastActiveData.current;
    if (!d?.activePayload?.length) return;
    // payload[0].payload.x est le plus fiable (activeLabel peut être undefined en Recharts v3)
    const raw = d.activePayload[0]?.payload?.x ?? d.activeLabel;
    const xd = +raw;
    if (!isFinite(xd) || xd < 0) return;
    // Y exact depuis l'équation du modèle actif
    const r = m.compute(p, Math.max(xd, 1e-9));
  ...
  }, [tool, activeModel, params, xAxis, yAxis, drawColor, m, p]);
  ...
  return (
    <div style={{ fontFamily: T.fontUi, background: T.bg, minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", minHeight: "calc(100vh - 62px)" }}>
        {/* Panneau gauche */}
        <div style={{ width: panelOpen ? 290 : 0, minWidth: panelOpen ? 290 : 0, transition: "width 0.22s ease, min-width 0.22s ease", overflow: "hidden", background: T.white, borderRight: `1px solid ${T.border}`, flexShrink: 0, position: "relative" }}>
          ...
          <div style={{ padding: 18, paddingTop: 44, width: 290, overflowY: "auto", height: "100%" }}>
